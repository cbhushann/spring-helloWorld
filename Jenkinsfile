  ...
  stages {
    ...
    stage('Health Check') {
      steps {
        script {
          def newColor = readFile('k8s/selected_color.txt').trim()
          def podName = sh(script: "kubectl get pods -l app=hello-world,version=${newColor} -o jsonpath='{.items[0].metadata.name}'", returnStdout: true).trim().replaceAll("'", "")
          sh "kubectl exec ${podName} -- curl -f http://localhost:8080/actuator/health"
        }
      }
    }
